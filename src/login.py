...
class ScutChargeMonitor:
    ...
    def __init__(
        ...
        try:
            self.llm_recognition_retries = (
                int(llm_recognition_retries)
                if llm_recognition_retries is not None
                else 1
            )
    ...
    def _custom_encrypt(self, plain_password):
        ...
        encrypted_password = "".join(
            char_map.get(char, char) for char in plain_password
        )
    ...
    def _recognize_captcha(self, captcha_image_base64):
        ...
        try:
            img_str = base64.b64encode(base64.b64decode(captcha_image_base64)).decode(
                "utf-8"
            )
            prompt_text = (
                "分析这张验证码图片。验证码由英文字母和数字构成，长度为严格的五个字符。请返回一个包含3个最可能结果的JSON数组，按可能性从高到低排序。"
                '例如：["abcd1", "abce3", "abcf2"]。请严格遵守JSON格式，不要包含任何其他说明文字。'
            )

            client = OpenAI(
                base_url=self.llm_api_base,
                api_key=self.llm_api_key,  # ModelScope Token
            )

            response = client.chat.completions.create(
                model=self.llm_model,  # ModelScope Model-Id, required
                messages=[
                    {
                        "role": "user",
                        "content": [
            ...
            first_message = (
                getattr(first_choice, "message", None) if first_choice else None
            )
            first_content = (
                getattr(first_message, "content", None) if first_message else None
            )
            ...
            logging.warning(
                f"无法将LLM响应解析为JSON，将把其作为唯一候选: {response_text}"
            )
    ...
    def _perform_auth_redirect(self):
        ...
        try:
            ...
            redirect_response = self.session.get(
                self.REDIRECT_URL, params=params, allow_redirects=False
            )
            ...
            dfyc_cookies = [
                ...
                and "dfyc.utc.scut.edu.cn" in cookie.domain
            ]
            ...
            if dfyc_cookies:
                ...
                js_preview = (
                    (self.jsessionid[:8] + "...")
                    if isinstance(self.jsessionid, str) and len(self.jsessionid) >= 8
                    else str(self.jsessionid)
                )
                ...
                if (
                    ...
                    if dfyc_cookies:
                        ...
                        js_preview = (
                            (self.jsessionid[:8] + "...")
                            if isinstance(self.jsessionid, str)
                            and len(self.jsessionid) >= 8
                            else str(self.jsessionid)
                        )
                        logging.info(f"成功在二次跳转后获取到 JSESSIONID: {js_preview}")
                        return True
                logging.warning(
                    "重定向请求完成，但未能找到 dfyc 域的 JSESSIONID cookie。"
                )
                return False
            ...
            if redirect_response:
                logging.debug(
                    f"失败时的Cookies: {redirect_response.cookies.get_dict()}"
                )
    ...
    def login(self):
        ...
        for round_idx in range(total_rounds):
            logging.info(
                f"=== 第 {round_idx + 1}/{total_rounds} 轮验证码识别与登录尝试开始 ==="
            )
            ...
            for i, captcha_code in enumerate(captcha_candidates):
                logging.info(
                    f"--- 正在使用第 {i + 1}/{len(captcha_candidates)} 个候选验证码 '{captcha_code}' 尝试登录 ---"
                )
                payload = {
                    "grant_type": "password",
                    "scope": "all",
                ...
                if response.status_code == 200:
                    if (
                        ...
                        logging.error(
                            f"登录响应不是有效的JSON格式: {snippet}...，错误原因：{parse_error or '缺少 access_token 字段'}"
                        )
                        continue
                    ...
                    token_preview = (
                        self.token[:8] + "..."
                        if isinstance(self.token, str) and len(self.token) >= 8
                        else str(self.token)
                    )
                ...
                if response.status_code == 400:
                    if error_code == 8002:
                        ...
                        logging.warning(
                            f"{context} 验证失败：{desc}。继续尝试下一个候选。"
                        )
                        continue
                    if error_code == 8000:
                        desc = message or "用户名或密码错误"
                        logging.error(
                            f"{context} 登录失败：{desc}。已确认账号/密码不匹配，后续不再重试。"
                        )
                        return False
                    friendly = message or "未知的业务错误"
                    logging.error(
                        f"{context} 登录请求返回 400：{friendly} (code={error_code})。"
                    )
                    continue
                ...
                if parse_error:
                    snippet = response.text[:200]
                    logging.error(
                        f"{context} 登录响应无法解析为JSON (HTTP {response.status_code})：{snippet}...，错误原因：{parse_error}"
                    )
                else:
                    snippet = response.text[:200]
                    logging.error(
                        f"{context} 登录请求发生未知HTTP错误，状态码: {response.status_code}, 响应: {snippet}..."
                    )
                continue

            logging.warning(
                "本轮所有验证码候选均已尝试失败，将进入下一轮重试（若仍有剩余次数）。"
            )
...
def main():
    ...
    if not all(
        config.get(k) for k in ["username", "password", "llm_model", "llm_api_key"]
    ):
        logging.error(
            "配置不完整，请检查 .env 文件。需要设置 SCUT_USERNAME, SCUT_PASSWORD, LLM_MODEL, LLM_API_KEY"
        )
        return
